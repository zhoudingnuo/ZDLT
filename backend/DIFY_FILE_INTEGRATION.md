...
- `apiKey`：Dify API 密钥
- `apiUrl`：Dify API 地址（例如：`http://118.145.74.50:24131/v1/chat-messages`）
- `inputType`：输入类型（dialogue/parameter）
- `inputs`：输入字段定义（包含文件类型）

### 文件上传地址

系统会自动根据 `apiUrl` 构建文件上传地址：
- 如果 `apiUrl` 包含 `/v1/chat-messages`，会自动替换为 `/v1/files/upload`
- 默认文件上传地址：`http://118.145.74.50:24131/v1/files/upload`
