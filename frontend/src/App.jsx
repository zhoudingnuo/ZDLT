          ...
          <div style={{ ...mainCardStyle, marginTop: 30 }}>
            <div style={chatContentStyle} ref={chatRef}>
              {messages.map((msg, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        {/* 仅AI回复显示计时，修复重复显示 */}
                        {msg.isLoading && !isUser && loading && idx === messages.length - 1 ? (
                          <span style={{ color: theme === 'dark' ? '#bbb' : '#888', fontSize: 13, marginLeft: 8 }}>
                            （{aiTimer}s）
                          </span>
                        ) : (!msg.isLoading && msg.usedTime && !isUser) ? (
                          <span style={{ color: theme === 'dark' ? '#bbb' : '#888', fontSize: 13, marginLeft: 8 }}>
                            （{msg.usedTime}s）
                          </span>
                        ) : null}
                        {/* 新增token标签，所有智能体都显示（除了小游戏） */}
                        {agent && !isUser && !msg.isLoading && (msg.tokens !== undefined) && (
                          <span style={{
                ...
                if (msg.isLoading && !isUser) {
                  return (
                    <div key={idx} style={{ display: 'flex', justifyContent: 'flex-start', margin: '8px 0' }}>
                      ...
                      >
                        <span>AI正在思考...</span>
                        <span style={{ marginLeft: 8, fontSize: 13, color: theme === 'dark' ? '#bbb' : '#888' }}>（{aiTimer}s）</span>
                      </div>
                    </div>
                  );
                }
                // 兜底调试
                return (
                  <pre key={idx} style={{ color: theme === 'dark' ? '#bbb' : '#888', fontSize: 13 }}>
